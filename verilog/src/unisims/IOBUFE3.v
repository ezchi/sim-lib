...
// begin behavioral model
    ...
    case (SIM_DEVICE)
      "ULTRASCALE", "ULTRASCALE_PLUS":
        case (USE_IBUFDISABLE)
          "TRUE":  
            assign O = (not_t_or_ibufdisable == 0)? (OSC_EN) ? O_OSC_in : IO : (not_t_or_ibufdisable == 1 && OSC_EN != 1)? 1'b0  : 1'bx;
          "FALSE": 
            assign O = (OSC_EN) ? O_OSC_in : IO;
          "T_CONTROL": 
            assign O = (not_t_or_ibufdisable == 1'b1) ? 1'b0 : (not_t_or_ibufdisable == 1'b0) ? IO : 1'bx;
        endcase
      default: 
        case (USE_IBUFDISABLE)
          "TRUE":  
  ...
  always @ (OSC or OSC_EN) begin
      OSC_int = OSC[2:0] * 5;
  if (OSC[3] == 1'b0 )
      OSC_int =  -1*OSC_int;
  
   if(OSC_EN == 1'b1) begin
    if ((SIM_INPUT_BUFFER_OFFSET_REG - OSC_int) < 0)
        O_OSC_in <= 1'b0;
    else if ((SIM_INPUT_BUFFER_OFFSET_REG - OSC_int) > 0)  
        O_OSC_in <= 1'b1;
