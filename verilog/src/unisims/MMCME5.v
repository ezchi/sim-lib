...
//  End Revision:
///////////////////////////////////////////////////////////////////////////////
//Fix for CR- 1155591 - change to logic when fb_delay < period_avg
...
`endif
  ...
  always @(posedge DCLK_in or posedge glblGSR) begin
    if (~glblGSR) begin
      if (drp_lock) begin
        if(~DEN_in) begin
          $display("Error: [Unisim %s-33] DEN is deasserted at time %t. Need wait for DRDY signal to be sampled high to comply with APB3 transfer. Instance %m ", MODULE_NAME, $time);
          #1 $finish;
        end
  ...
  if (fps_en == 1) begin
    ...
    if (ps_cnt0 == 0)
      period_ps0 = 0;
    else if (ps_cnt0 < 0 )//|| (ps_cnt0 == 0 && PSINCDEC_in == 0))
  ...
  if (fps_en == 1) begin
    ...
    if (ps_cnt1 == 0)
      period_ps1 = 0;
    else if (ps_cnt1 < 0 || (ps_cnt1 == 0 && PSINCDEC_in == 0))
  ...
  if (fps_en == 1) begin
    ...
    if (ps_cnt2 == 0)
      period_ps2 = 0;
    else if (ps_cnt2 < 0 || (ps_cnt2 == 0 && PSINCDEC_in == 0))
  ...
  if (fps_en == 1) begin
    ...
    if (ps_cnt3 == 0) 
      period_ps3 = 0;
    else if (ps_cnt3 < 0 || (ps_cnt3 == 0 && PSINCDEC_in == 0))
  ...
  if (fps_en == 1) begin
    ...
    if (ps_cnt4 == 0)
      period_ps4 = 0;
    else if (ps_cnt4 < 0 || (ps_cnt4 == 0 && PSINCDEC_in == 0))
      period_ps4 = (period_vco_rl*clk4_div) + ps_cnt4 * period_vco_rl / (ps_res*1.0); 
  ...
  if (fps_en == 1) begin
    ...
    if (ps_cnt5 == 0)
      period_ps5 = 0;
    else if (ps_cnt5 < 0 || (ps_cnt5 == 0 && PSINCDEC_in == 0))
      period_ps5 = (period_vco_rl*clk5_div) + ps_cnt5 * period_vco_rl / (ps_res*1.0); 
  ...
  if (fps_en == 1) begin
    ...
    if ((ps_cnt6 == 0) && PSINCDEC_in == 0)
      period_ps6 = 0;
    else if (ps_cnt6 < 0 || (ps_cnt6 == 0 && PSINCDEC_in == 0))
      period_ps6 = (period_vco_rl*clk6_div) + ps_cnt6 * period_vco_rl / (ps_res*1.0); 
  ...
  if (fps_en == 1) begin
    ...
    if (ps_cnt_fb == 0)
      period_ps_fb = 0;
    else if (ps_cnt_fb < 0 || (ps_cnt_fb == 0 && PSINCDEC_in == 0))
      period_ps_fb = (period_vco_rl*divclk_div) + ps_cnt_fb * period_vco_rl / (ps_res*1.0); 
  ...
  always @ (*) begin
    if (fb_delay_found == 1) begin
      if (fb_delay == 0) begin
        ...
        clkpll_delay =  period_avg - (period_avg % fb_delay);
      end else begin
  ...
  always @(posedge PSCLK_in or posedge rst_int)
    ...
    if (PSEN_in) begin
      ...
      psen_w <= 1;
        ...
        if ( ps_cnt4 > $rtoi($itor(((-1)*(ps_res*clk4_div-1)))))        ps_cnt4   <= ps_cnt4 - 1; 
        else                                                            ps_cnt4   <= 0;           
        if ( ps_cnt5 > $rtoi($itor(((-1)*(ps_res*clk5_div-1)))))        ps_cnt5   <= ps_cnt5 - 1; 
        else                                                            ps_cnt5   <= 0;           
        if ( ps_cnt6 > $rtoi($itor(((-1)*(ps_res*clk6_div-1)))))        ps_cnt6   <= ps_cnt6 - 1; 
        else                                                            ps_cnt6   <= 0;           
        if ( ps_cnt_fb > $rtoi($itor(((-1)*(ps_res*divclk_div-1)))))    ps_cnt_fb <= ps_cnt_fb - 1;
        else                                                            ps_cnt_fb <= 0;           
 ...
 always  @(negedge clkvco or posedge rst_in_o)
  ...
  always @(posedge clk4in or negedge clk4in or posedge rst_in_o)
    if (rst_in_o || ~clk4ps_en || ~pll_locked) begin
      ...
      if (clk4_cnt < clk4_cnt_max) begin
        clk4_cnt <= clk4_cnt + 1;
        psdec_adj4_done<=1'b0;
      end
      else begin
  ...
  always @(posedge clk5in or negedge clk5in or posedge rst_in_o)
    if (rst_in_o || ~clk5ps_en || ~pll_locked) begin
      ...
      if (clk5_cnt < clk5_cnt_max) begin
        clk5_cnt <= clk5_cnt + 1;
        psdec_adj5_done<=1'b0;
      end
      else begin
  ...
  always @(posedge clk6in or negedge clk6in or posedge rst_in_o)
    if (rst_in_o || ~clk6ps_en || ~pll_locked) begin
      ...
      if (clk6_cnt < clk6_cnt_max) begin
        clk6_cnt <= clk6_cnt + 1;
        psdec_adj6_done<=1'b0;
      end
      else begin
