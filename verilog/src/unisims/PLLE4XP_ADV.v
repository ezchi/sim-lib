...
`ifdef XIL_TIMING
  ...
  parameter real VCOCLK_FREQ_MIN = 537.500,
`endif
  ...
  parameter DMC_CLOCK = "FALSE",
  parameter [0:0] IS_CLKFBIN_INVERTED = 1'b0,
  parameter [0:0] IS_CLKIN_INVERTED = 1'b0,
  parameter [0:0] IS_PWRDWN_INVERTED = 1'b0,
  parameter [0:0] IS_RST_DMC_INVERTED = 1'b0,
...
`ifndef XIL_TIMING
  ...
  localparam real VCOCLK_FREQ_MIN = 537.500;
`endif

// define constants
  localparam MODULE_NAME = "PLLE4XP_ADV";
...
// Parameter encodings and registers
  ...
  localparam COMPENSATION_PHY_ALIGN = 3;
  localparam DMC_CLOCK_FALSE = 0;
  localparam DMC_CLOCK_TRUE = 1;
...
`else
  ...
  reg [72:1] COMPENSATION_REG = COMPENSATION;
  reg [31:0] DIVCLK_DIVIDE_REG = DIVCLK_DIVIDE;
  reg [40:1] DMC_CLOCK_REG = DMC_CLOCK;
  ...
  reg [0:0] IS_RST_DMC_INVERTED_REG = IS_RST_DMC_INVERTED;
...
`endif
  ...
  assign CLKOUTPHY_DMCEN_in = CLKOUTPHY_DMCEN;
  ...
  assign RST_DMC_in = RST_DMC ^ IS_RST_DMC_INVERTED_REG;
...
//  localparam O_MIN = 1; // not used
  ...
  reg clkvco_lk = 1'b0, clkvco_lk_tmp = 1'b0, clkvco = 1'b0, clkvco_by2 = 1'b0, clkvco_2x = 1'b0, clkvco_dmc = 1'b0;
  reg clkvco_by8 = 1'b0;
  ...
  wire [1:0] hs_div = 0;
  wire dmcen_to_sync;
  reg mc_dmcen_override = 0;
  wire CLKOUTPHY1;
  reg mc_hs_path_sel = 0;
  wire gated_hs_clk; 
  reg sync1, sync2, sync3;
  reg a_sync1, a_sync2, a_sync3;
  wire dmcen_sync_out0, dmcen_sync_out1;
  reg clkhs_in, clkhs_by_2;
  reg clk_hs_reg;
  wire outp; 
  reg vco_div_by_2;
  reg mc_hs_div_enable = 0;
`ifdef XIL_TIMING
  reg notifier;
`endif
...
`endif
  ...
  wire clkfbm1ps_en;
  reg sel_bw_en_dmcen = 1; //en_rkshort //Logic added for xp
  reg drp_updt = 1'b0;
  ...
  always @(*)
  begin
  if (CLKOUTPHY_MODE_BIN == CLKOUTPHY_MODE_VCO_DMC)
  begin
      sel_bw_en_dmcen = 0;
  end
  end
  assign xiphyen_in = sel_bw_en_dmcen ? CLKOUTPHYEN : CLKOUTPHY_DMCEN; // Logic added for xp
  ...
  assign rst_input_r =  (RST ^ IS_RST_INVERTED_REG) | (RST_DMC ^ IS_RST_DMC_INVERTED); //Logic added for xp
  //assign rst_input_r =  (RST ^ IS_RST_INVERTED_REG) | (RST_DMC_in);
  assign LOCKED = locked_out1;
  assign DRDY = drdy_out1;
  assign DO = do_out1;
  assign LOCKED_DMC = locked_out1;
...
`ifdef XIL_XECLIB
  ...
  assign DMC_CLOCK_BIN =
      (DMC_CLOCK_REG == "FALSE") ? DMC_CLOCK_FALSE :
      (DMC_CLOCK_REG == "TRUE") ? DMC_CLOCK_TRUE :
       DMC_CLOCK_FALSE;
...
`ifndef XIL_XECLIB
  always @ (trig_attr) begin
    ...
    if ((attr_test == 1'b1) ||
      ...
      $display("Error: [Unisim %s-112] CLKOUTPHY_MODE attribute is set to %s.  Legal values for this attribute are VCO_2X, VCO, VCO_DMC or VCO_HALF. Instance: %m", MODULE_NAME, CLKOUTPHY_MODE_REG);
      attr_err = 1'b1;
    end
    
    if ((attr_test == 1'b1) ||
        (CLKPFD_FREQ_MAX_REG < 667.500 || CLKPFD_FREQ_MAX_REG > 667.500)) begin
      $display("Error: [Unisim %s-113] CLKPFD_FREQ_MAX attribute is set to %f.  Legal values for this attribute are 667.500 to 667.500. Instance: %m", MODULE_NAME, CLKPFD_FREQ_MAX_REG);
      attr_err = 1'b1;
    end
    ...
    if ((attr_test == 1'b1) ||
      ...
      $display("Error: [Unisim %s-115] COMPENSATION attribute is set to %s.  Legal values for this attribute are AUTO, BUF_IN, INTERNAL or PHY_ALIGN. Instance: %m", MODULE_NAME, COMPENSATION_REG);
      attr_err = 1'b1;
    end
    ...
    if ((attr_test == 1'b1) ||
        ((DMC_CLOCK_REG != "FALSE") &&
         (DMC_CLOCK_REG != "TRUE"))) begin
      $display("Error: [Unisim %s-117] DMC_CLOCK attribute is set to %s.  Legal values for this attribute are FALSE or TRUE. Instance: %m", MODULE_NAME, DMC_CLOCK_REG);
      attr_err = 1'b1;
    end
    
    if ((attr_test == 1'b1) ||
        (REF_JITTER_REG < 0.000 || REF_JITTER_REG > 0.999)) begin
      $display("Error: [Unisim %s-123] REF_JITTER attribute is set to %f.  Legal values for this attribute are 0.000 to 0.999. Instance: %m", MODULE_NAME, REF_JITTER_REG);
      attr_err = 1'b1;
    end
    ...
    if ((attr_test == 1'b1) ||
      ...
      $display("Error: [Unisim %s-124] STARTUP_WAIT attribute is set to %s.  Legal values for this attribute are FALSE or TRUE. Instance: %m", MODULE_NAME, STARTUP_WAIT_REG);
      attr_err = 1'b1;
    end
    
    if ((attr_test == 1'b1) ||
        (VCOCLK_FREQ_MAX_REG < 1600.000 || VCOCLK_FREQ_MAX_REG > 1600.000)) begin
      $display("Error: [Unisim %s-125] VCOCLK_FREQ_MAX attribute is set to %f.  Legal values for this attribute are 1600.000 to 1600.000. Instance: %m", MODULE_NAME, VCOCLK_FREQ_MAX_REG);
      attr_err = 1'b1;
    end
    
    if ((attr_test == 1'b1) ||
        (VCOCLK_FREQ_MIN_REG < 537.500 || VCOCLK_FREQ_MIN_REG > 537.500)) begin
      $display("Error: [Unisim %s-126] VCOCLK_FREQ_MIN attribute is set to %f.  Legal values for this attribute are 537.500 to 537.500. Instance: %m", MODULE_NAME, VCOCLK_FREQ_MIN_REG);
      attr_err = 1'b1;
    end
    ...
    if ((attr_test == 1'b1) ||
      ...
      $display("Error: [Unisim %s-127] VCO_RANGE attribute is set to %s.  Legal values for this attribute are HIGHRANGE or LOWRANGE. Instance: %m", MODULE_NAME, VCO_RANGE_REG);
      attr_err = 1'b1;
    end
...
//  assign clkoutxiphy_int = xiphy_mode[1] ? (xiphy_mode[0] ? clkind_out : clkvco_by2) : (xiphy_mode[0] ? clkvco : clkvco_2x);
  //assign clkoutxiphy_int = CLKOUTPHY_MODE_BIN[1] ? clkvco_by2 : (CLKOUTPHY_MODE_BIN[0] ? clkvco : clkvco_2x);

  assign hs_div[1] = VCO_RANGE_BIN; //Logic added for xp

  assign clkoutxiphy_int = hs_div[1] ? (CLKOUTPHY_MODE_BIN[1] ? (CLKOUTPHY_MODE_BIN[0] ? clkvco_by2 : clkvco_dmc) : (CLKOUTPHY_MODE_BIN[0] ? clkvco : clkvco_2x)) : (CLKOUTPHY_MODE_BIN[1] ? clkvco_by2 : (CLKOUTPHY_MODE_BIN[0] ? clkvco : clkvco_2x)) ; //Logic added for xp

  assign CLKOUTPHY1 = xiphyen_sync & clkoutxiphy_int; //Logic Added for xp Chaging clkoutphy to clkoutphy1 and assigning at last
//xiphy_mode has been changed to CLKOUTPHY_MODE_BIN
  assign  glock = (STARTUP_WAIT_BIN) ? locked_out_tmp : 1;    //startup_wait_sig is changed to  STARTUP_WAIT_BIN         // Are these needed
  assign (weak1, strong0) glbl.PLL_LOCKG = (glock == 0) ? 0 : p_up;

//Logic added for xp clkoutphy_dmcen
assign dmcen_to_sync = mc_dmcen_override ? 1'b1 : CLKOUTPHY_DMCEN;
...
/*  always @ *
  begin 
    clk_hs_reg <= clkvco_lk_tmp; // ~vco_count_2_int
  end

assign clkhs_in = clk_hs_reg;*/

always @(posedge clkvco) //clk_hs_reg is clkvco
  begin
    vco_div_by_2 <= ~ vco_div_by_2;
  end

assign outp = mc_hs_div_enable ?  vco_div_by_2 : clkvco_lk_tmp; // clk_hs_reg is temporary variable connected to clkvco

always @(posedge clkvco) //clkhs_in 
  begin
    clkhs_by_2 <= ~ clkhs_by_2;  //This is done as synchronizers wont work on high speed freq of 4.3GHz
  end

always @ (posedge clkhs_by_2 or posedge rst_in) //rn may be RST
    if (rst_in) begin
        sync1 = 1'b0;
        sync2 = 1'b0;
        sync3 = 1'b0;
...
assign dmcen_sync_out0 = sync3;

always @ (posedge clkhs_by_2 or negedge rst_in)
    if(rst_in) begin
        a_sync1 = 1'b0;
        a_sync2 = 1'b0;
        a_sync3 = 1'b0;
        ...
        a_sync1 = dmcen_sync_out0;
        a_sync2 = a_sync1;
        a_sync3 = a_sync2;
    end
assign dmcen_sync_out1 = a_sync3;

assign gated_hs_clk = clkvco & dmcen_sync_out1; //clkhs_in is clkvco

//assign clkoutphy_p = mc_hs_path_sel ? gated_hs_clk : clkphy_in;
assign CLKOUTPHY = mc_hs_path_sel ? gated_hs_clk : CLKOUTPHY1;


///Logic ends here for xp
initial
begin
    init_chk = 0;
    #2;
...
// Determine feedback delay - only internal feedback
  ...
  begin
    if (rst_in)
      ...
      clkvco  = outp; //clkvco_lk_tmp; //Logic added for xp
  end  
...
// VCO clock generation
  ...
  begin
    ...
    begin
      clkout_mux[0] = clkvco;
      clkout_mux[1] = clkvco; //CR-1159901,1200237 Changing Non_blocking to blocking
      clkout_mux[2] = clkvco;
    end
  end  
...
// end of lock	
  ...
  always @(clk0in or posedge rst_in)
  begin
    if (rst_in)
    begin
        clk0_cnt = 9'b0;
  ...
  always @(clk1in or posedge rst_in)
  begin
    if (rst_in)
    begin
        clk1_cnt = 9'b0;
  ...
  always @(clkpll or posedge rst_in)
  begin
    if (rst_in) begin
        clkind_cnt = 8'b0;
        clkind_out = 0;
  ...
  always @(clkfbm1in or posedge rst_in)
  begin
    if (rst_in) begin
        clkfbm1_cnt = 8'b0;
        clkfbm1_nf_out = 0;
  ...
  always @(CLKFBIN_int or posedge rst_in)
  begin
    if (rst_in) begin
        clkfbm2_cnt = 8'b0;
        clkfbm2_out = 0;
...
always @(posedge clkhs_in)
  begin
    clkhs_by_2 <= ~ clkhs_by_2;
  end
// Phase task
  task clkout_dly_cal;
