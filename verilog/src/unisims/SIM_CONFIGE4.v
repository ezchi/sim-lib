...
//   /   /                      SIM_CONFIGE4 support Spartan UltraScale+ device SelectMAP write only
//  /   /                       Versal SoC SelectMAP is not supported.
// /___/   /\     Filename    : SIM_CONFIGE4.v
// \   \  /  \
//  \___\/\___\
//
///////////////////////////////////////////////////////////////////////////////
...
`endif
  ...
  output AVAIL,      //only for ICAP
  output CSOB,       //only for Artix/Kintex/Virtex UltraScale+ device SelectMAP
  output PRDONE,     //only for ICAP
  output PRERROR,    //only for ICAP
  output BUSY,       //BUSY is only for Spartan UltraScale+ device SelectMAP
  inout  DONE,
  input  CCLK,
  input  CSB,
  inout  [31:0] D,
...
//  assign (weak1, strong0) DONE=  (bout_en[3] == 0) || done_o[3];
  ...
  localparam MODULE_NAME = "SIM_CONFIGE4";
                 ...
                 tmp_byte = bit_revers8(d_in[7:0]);
                  if (buswid_flag_init[ib] == 0) begin
                     if (tmp_byte == 8'hBB)
                      ...
                      if (buswid_flag[ib] == 0) begin
                         if (tmp_byte == 8'h88)  begin // x8 SUS+ CR- 1204899 Buswidth pattern.
                                 buswidth_tmp[ib] <= 2'b01;
                         end    
                         else if (buswidth_tmp[ib] == 2'b01 && tmp_byte == 8'h44) begin
                                 buswid_flag[ib] <= 1; 
                                 buswidth_tmp[ib] <= 2'b01;
                         end
                         else if (tmp_byte == 8'h44) begin  // x16 SUS+
                                 buswidth_tmp[ib] <= 2'b10;
                         end
                         else if (buswidth_tmp[ib] == 2'b10 && tmp_byte == 8'h22) begin
                                 buswid_flag[ib] <= 1; 
                                 buswidth_tmp[ib] <= 2'b10;
                         end
                         else if (tmp_byte == 8'h22) begin   // x32 SUS+i
                                 buswidth_tmp[ib] <= 2'b11;
                         end
                         else if (buswidth_tmp[ib] <= 2'b11 && tmp_byte == 8'h11) begin
                                 buswid_flag[ib] <= 1; 
                                 buswidth_tmp[ib] <= 2'b11;
                         end
                      end
                         ...
                         if (icap_on == 0)
                         $display("Error: [Unisim %s-6] BUS Width Auto Detection did not find 0x11 or 0x22 or 0x44 after 0x22 or 0x44 or 0x88 on D[7:0] followed 0xBB at time %t. Instance %m.", MODULE_NAME, $time);
                         else 
                         $display("Error: [Unisim %s-7] BUS Width Auto Detection did not find 0x44 on dix[7:0] followed 0xBB on ICAPE3 instance at time %t. Instance %m.", MODULE_NAME, $time);
...
initial
begin
    if(DEVICE_ID == "04E80093" || DEVICE_ID == "04E81093" || DEVICE_ID == "04E82093" || DEVICE_ID == "04E88093" || DEVICE_ID == "04E90093" || DEVICE_ID == "04E98093" || DEVICE_ID == "04E99093" || DEVICE_ID == "04EA0093" || DEVICE_ID == "04EA1093") begin
    if (prog_b_in == 0 || DONE == 1)
        BUSY_out <= 1'bz;
    else if (init_b_t == 1 || mode_sample_flag == 1)
        BUSY_out <= 1'b1 ;
end
else begin
    BUSY_out <= 1'b0;
...
always @ (posedge cclk_in)
    if (BUSY_out == 1) begin
        if (BUSY_COUNT < 50 ) begin
        BUSY_COUNT = BUSY_COUNT + 1;
        end
        else if (BUSY_COUNT == 50) begin
        BUSY_out <= 1'b0;
        end
    end
...
always @ (posedge cclk_in)
    ...
    else if(csi_b_in == 0 && rdwr_b_in == 0 && CSB_COUNT < 1024) begin
        CSB_COUNT = CSB_COUNT + 1;
    end
    else if (csi_b_in == 0 && rdwr_b_in == 0 && CSB_COUNT == 1024) begin
        BUSY_out <= 1'b1;
        CSB_COUNT = 0;
    end
    else if (csi_b_in == 0 && rdwr_b_in == 0 && BUSY_out == 1 && CSB_COUNT == 25) begin
        $display("Error: [Unisim %s-2] input data FIFO has overflowed. Instance: %m", MODULE_NAME);
    end


    ///////end of code for CR -1204899
