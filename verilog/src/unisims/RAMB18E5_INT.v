      ...
      if (glblGSR || INIT_MEM) begin
        ...
        if (~wr_b_data_matches_rd_a_data &&
         ...
         for (i=0;i<D_WIDTH;i=i+1) begin
             if ( mem_we_b[i]==1 ) begin
                 mem_a_lat[i] <= #100 1'bx;
             end
             else
               begin
                  if (wr_b_rd_a_addr_coll && coll_win_wr_clk_b_min && sdp_mode && (WRITE_MODE_B_BIN == WRITE_MODE_B_READ_FIRST)) begin
                     mem_a_lat[i]   <= #100 mem_rd_b_rf[i];
                  end
                  else
                    begin
                     mem_a_lat[i]   <= #100 mem_rd_a[i];
                    end
               end
         end
         for (i=0;i<DP_WIDTH;i=i+1) begin
             if ( memp_we_b[i]==1 ) begin
                 memp_a_lat[i] <= #100 1'bx;
             end
             else
               begin
                  if (wr_b_rd_a_addr_coll && coll_win_wr_clk_b_min && sdp_mode && (WRITE_MODE_B_BIN == WRITE_MODE_B_READ_FIRST)) begin
                     memp_a_lat[i]  <= #100 memp_rd_b_rf[i];
                  end
                  else
                    begin
                     memp_a_lat[i]  <= #100 memp_rd_a[i];
                    end
               end
         end        
        end
   ...
   always @(posedge CLKBWRCLK_in or posedge INIT_MEM or posedge wr_a_rd_b_coll or posedge rd_b_wr_a_coll or posedge wr_a_wr_b_coll or posedge  wr_b_wr_a_coll or posedge mem_arst_b) begin
      if (glblGSR || INIT_MEM) begin
        ...
        if (~wr_a_data_matches_rd_b_data &&
            ((SIM_COLLISION_CHECK_BIN == SIM_COLLISION_CHECK_ALL) ||
             (SIM_COLLISION_CHECK_BIN == SIM_COLLISION_CHECK_GENERATE_X_ONLY))) begin
         for (i=0; i< D_WIDTH/2 ; i = i+1)
         begin
             if (mem_we_a[i] == 1) begin
                 mem_b_lat[i] <= #100 1'bx;
             end
             else
              begin
                 mem_b_lat[i] <= #100 mem_rd_b[i];
              end
         end
         for (i=0; i< DP_WIDTH/2 ; i = i+1)
         begin
             if (memp_we_a[i] == 1) begin
                 memp_b_lat[i] <= #100 1'bx;
             end
             else
              begin
                 memp_b_lat[i] <= #100 memp_rd_b[i];
              end
         end
        end
...
// write engine
   always @ (posedge CLKARDCLK_in or posedge wr_a_wr_b_coll or posedge wr_b_wr_a_coll or posedge glblGSR or posedge mem_arst_a) begin
     ...
     if ((wr_a_wr_b_coll || wr_b_wr_a_coll) && ~(glblGSR || INIT_MEM)) begin
       if (~wr_data_matches) begin
         mem_rd_a_wf = {D_WIDTH{1'bx}};
         memp_rd_a_wf = {DP_WIDTH{1'bx}};
   ...
   always @ (posedge CLKBWRCLK_in or posedge INIT_MEM or posedge wr_b_wr_a_coll or posedge wr_a_wr_b_coll or posedge glblGSR or posedge mem_arst_b) begin
...
// initialize memory p
     ...
     if ((wr_b_wr_a_coll || wr_a_wr_b_coll) && ~(glblGSR || INIT_MEM)) begin
       if (~wr_data_matches) begin
       mem_rd_b_wf = {D_WIDTH/2{1'bx}};
       memp_rd_b_wf = {DP_WIDTH/2{1'bx}};
