...
`endif
  ...
  parameter RESAMPLE_RATE = "7/2",
  parameter integer XPA_CFG0 = 0
)(
  output [223:0] M_AXIS_DOUT_TDATA,
  output [6:0] M_AXIS_DOUT_TKEEP,
...
// define constants 
 localparam MODULE_NAME = "DFE_PQ_INTERP";
  
  reg trig_attr;
// include dynamic registers - XILINX test only
`ifdef XIL_DR
  `include "DFE_PQ_INTERP_dr.v"
`else
...
`endif
  ...
  wire [223:0] M_AXIS_DOUT_TDATA_out;
  wire [6:0] M_AXIS_DOUT_TKEEP_out;
...
`ifndef XIL_XECLIB
  always @ (trig_attr) begin
    ...
    if ((attr_test == 1'b1) ||
        ((RESAMPLE_RATE_REG != "7/2") &&
         (RESAMPLE_RATE_REG != "2/1") &&
         (RESAMPLE_RATE_REG != "3/1") &&
         (RESAMPLE_RATE_REG != "3/2") &&
         (RESAMPLE_RATE_REG != "4/1") &&
         (RESAMPLE_RATE_REG != "5/1") &&
         (RESAMPLE_RATE_REG != "5/2") &&
         (RESAMPLE_RATE_REG != "7/1"))) begin
      $display("Error: [Unisim %s-103] RESAMPLE_RATE attribute is set to %s.  Legal values for this attribute are 7/2, 2/1, 3/1, 3/2, 4/1, 5/1, 5/2 or 7/1. Instance: %m", MODULE_NAME, RESAMPLE_RATE_REG);
      attr_err = 1'b1;
    end
...
`ifndef XIL_XECLIB
  SIP_DFE_PQ_INTERP SIP_DFE_PQ_INTERP_INST (
